...
class Requests extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('udid, productid, lastupdated', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('udid', 'length', 'max'=>40),
			array('productid', 'length', 'max'=>200),
			array('email', 'length', 'max'=>100),
			array('message', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, udid, productid, email, message, status, lastupdated', 'safe', 'on'=>'search'),
		);
	}
