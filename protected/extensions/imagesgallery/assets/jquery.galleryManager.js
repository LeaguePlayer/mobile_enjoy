(function ($) {
    ...
    function galleryManager(el, options) {
        ...
        function createEditorElement(id, src, name, description, tags) {
            var html = '<div class="photo-editor">' +
                ...
                (opts.hasDesc
                    ...
                    : '') +
                    '<label for="photo_id_'+id+'">Теги ( Услуги )</label>' +
                    '<input type="hidden" id="photo_id_'+id+'" class="tags" name="photo['+ id +'][tags]" value="'+tags+'">'+
                '</div>' +
                '</div>';
            return $(html);
        }
        ...
        photoTemplate += '<span class="mainPhoto btn btn-success btn-mini"><i class="icon-star icon-white"></i></span> ';
        ...
        function editPhotos(ids) {
            ...
            for (var i = 0; i < l; i++) {
                ...
                form.append(createEditorElement(id, src, name, description, tags));
            }
        ...
        function mainClick(e) {
            ...
            if(!$(this).hasClass('disabled')){
                ...
                images.find('.mainPhoto').removeClass('disabled');
                $(this).addClass('disabled');

                $.ajax({
                type: 'POST',
                url: opts.changeMainUrl,
                data: 'main_id='+main_id+'&id[]=' + ids.join('&id[]=') + csrfParams,
                success: function (t) {
            ...
            function multiUpload(files) {
                ...
                for (var i = 0; i < filesCount; i++) {
                    ...
                    xhr.onload = function () {
                        ...
                        if($images.find('span.disabled').length == 0){
                            $images.find('.photo').eq(0).find('.mainPhoto').click();
                        }
                    };
                    xhr.send(fd);
                }
        ...
        $.each(opts.tags,function(key,val){
            $.each(val,function(key_r,val_r){
                data.push({
                    id:val_r.id,
                    text:val_r.text
                });
        ...
        for (var i = 0, l = opts.photos.length; i < l; i++) {
            ...
            var data=[];
            $.each(resp['tags'],function(key,val){
                data.push(key);
            })
            addPhoto(resp['id'], resp['preview'], resp['name'], resp['description'], resp['rank'], resp['main'],data);
        }
    }
